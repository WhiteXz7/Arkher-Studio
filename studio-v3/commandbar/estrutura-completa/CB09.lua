--[[ =====================================================================
  ARKHER V3 — ESTRUTURA COMPLETA (9/13)
  Server — ArkherKit_Installer_B
  Cria: ServerScriptService.Arkher.ArkherKit_Installer_B (Script desativado) + ServerStorage.ArkherData
  Uso: View > Command Bar > cole TODO este texto > Run
  Idempotente: pode rodar de novo (apenas atualiza o Source)
====================================================================== ]]
local sss = game:GetService("ServerScriptService")
local arkher = sss:FindFirstChild("Arkher")
if not arkher then arkher = Instance.new("Folder") arkher.Name = "Arkher" arkher.Parent = sss end
local S_INST = [=====[--[[ ARKHER V3 — Installer ArkherKit_B: cria o ModuleScript em ReplicatedStorage.ArkherV3 ]]
local KIT = [====[--[[ ARKHER V3 — KIT B (ModuleScript) — places + actions + singularity + live + boot ]]
...
local t = arkher:FindFirstChild("ArkherKit_Installer_B")
if not t then t = Instance.new("Script") t.Name = "ArkherKit_Installer_B" t.Parent = arkher end
t.Source = S_INST
	t.Disabled = true
local ss = game:GetService("ServerStorage")
local arkherdata = ss:FindFirstChild("ArkherData")
if not arkherdata then arkherdata = Instance.new("Folder") arkherdata.Name = "ArkherData" arkherdata.Parent = ss end
print("[ARKHER V3] (9/13) ServerStorage.ArkherData criado (namespace de dados do servidor)")
print("[ARKHER V3] (9/13) ArkherKit_Installer_B guardado em ServerScriptService.Arkher (desativado)")
