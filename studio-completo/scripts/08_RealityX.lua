...
function BUILD.planeta()
	...
	section("TECH Z ORIGINAL (Reality Layer / Predictive)", t.err)
	toolBtn("Reality Layer", "5 camadas vivas (GEO/HYDRO/ATMO/BIO/URB) compondo cada célula — realidade, não terreno pelado", function()
		return remoteMsg("rl_build", {})
	end)
	toolBtn("Stats Camadas", "Células por camada, áreas urbanas, tints aplicados", function()
		return remoteMsg("rl_stats", {})
	end)
	toolBtn("🔮 Predictive ON", "D-O15 Predictive: o mundo materializa ANTES da sua percepção chegar (EMA velocidade+aceleração)", function()
		return remoteMsg("pred_on", { on = true })
	end)
	toolBtn("Stats Predictive", "Horizonte por tier, alvo previsto, hotspots de atenção aprendidos", function()
		return remoteMsg("pred_stats", {})
	end)
...
function BUILD.vida()
	...
	section("MENTES (cat. K — NMN recodificado RRW)", t.purple)
	toolBtn("🧠 Mentes: o que pensam", "Objetivos por mente (rest/seek_food/seek_people/wander) com necessidades reais", function()
		return remoteMsg("mind_stats", {})
	end)
	toolBtn("Por que decidiu isso?", "Memória com CAUSA — cada decisão auditável (a tese exige 'porquê')", function()
		return remoteMsg("mind_why", {})
	end)
	toolBtn("Comida aqui (0,0)", "Ponto de alimento: mentes famintas vão buscar sozinhas", function()
		return remoteMsg("mind_food", { x = 0, z = 0 })
	end)
